  ...
  <div>
    <div class="details" :key="'details_'+dtKey">
      <v-row v-if="can_edit && !showForm" no-gutters>
        <v-col class="d-flex pa-0">
          ...
          <v-icon v-if="!showForm" title="Edit Platform Settings" @click="showForm=true">mdi-cog-outline</v-icon>
          &nbsp;
          <v-icon v-if="is_admin && mutable_prov.can_delete" title="Delete Platform" @click="destroy(mutable_prov.id)">
      ...
      <!-- Values-only when form not active -->
        ...
        <tr>
	        ...
	        <td>
              ...
              <v-chip v-if="cnx.required">{{ cnx.name }}</v-chip>
            </template>
	        </td>
	      </tr>
    ...
    <div class="related-list">
      <v-expansion-panels><v-expansion-panel>
          ...
          <div v-else>
            <p>No harvest records found for this platform</p>
          </div>
  	    </v-expansion-panel-content>
	    </v-expansion-panel></v-expansion-panels>
    </div>
    <div class="related-list">
      <h2 v-if="is_admin" class="section-title">COUNTER API Credentials by Institution </h2>
      <h2 v-else class="section-title">COUNTER API Credentials</h2>
      <div v-if="is_admin">
        <v-row class="d-flex mb-4" no-gutters>
          <v-col class="d-flex pa-0" cols="3">
            <v-btn small color="primary" type="button" @click="enableImportDialog" class="section-action">
              Import COUNTER API Credentials
            </v-btn>
          </v-col>
          <v-col class="d-flex px-1" cols="3">
      ...
      <div v-if="(is_manager || is_admin) && mutable_unset.length > 0">
        <form method="POST" action="" @submit.prevent="sushiFormSubmit"
          ...
          <div v-if="showSushiForm" class="form-fields">
            <template v-for="cnx in connectors">
              <v-text-field v-if="cnx.required" v-model="sushiForm[cnx.name]" :label='cnx.label' :id='cnx.name' outlined
              ></v-text-field>
              &nbsp; &nbsp;
            </template>
      ...
      <v-data-table :headers="headers" :items="mutable_prov.sushiSettings" item-key="id" :key="'setdt_'+dtKey">
        <template v-slot:item="{ item }" >
          <tr>
            ...
            <td v-if="connectors.some(c => c.name === 'customer_id') && c.required">
        ...
        <v-card>
          ...
          <v-card-text>
            <v-container grid-list-md>
              ...
              ></v-file-input>
              <p>
                <strong>Note:&nbsp; COUNTER API Credentials imports function exclusively as Updates. No existing credentials
    ...
    export default {
        ...
        methods: {
            ...
            destroy (provid) {
                Swal.fire({
                  title: 'Are you sure?',
                  text: "Deleting a platform cannot be reversed, only manually recreated."+
                        " Because this platform has no harvested usage data, it can be safely"+
                        " deleted. NOTE: All COUNTER API credentials connected to this platform"+
                ...
                this.sushiForm.post('/sushisettings')
	                .then((response) => {
                      if (response.result) {
                          ...
                          this.sushiForm.prov_id = this.provider.id;
