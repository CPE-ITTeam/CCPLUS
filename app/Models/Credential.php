...
class Credential extends Model
{
   ...
   */
    ...
    {
        return $this->hasMany('App\Models\HarvestLog', 'credentials_id');
    }
    ...
    {
        return $this->belongsTo('App\Models\HarvestLog', 'last_harvest_id');
    }
    ...
    {
        // if not authorized or credential is Disabled, bail out silently
        ...
        if ($this->isComplete() || $new_status == 'Suspended') {
            // Inst+Prov active and credentials complete sets to "Enabled", otherwise sets to "Suspended"
            $this->status = $new_status;
        // Getting here means inst+prov are both Active but credentials are incomplete. update the field values
        } else {
            $required = $this->provider->connectors();
