...
class CleanupJson extends Command
{
    ...
    public function handle()
    {
        ...
        // (any failed harvests, error-attempts, or "Success" with a 3030/9030)
        // Will also tidy-up FailedHarvest records for Success status while we're here
        $movedCount = 0;
        $deletedCount = 0;
        foreach ($all_harvests as $harvest) {
            $deleteFile = false;
            if ($harvest->status == 'Success') {
                if ($harvest->failedHarvests->count() > 0) {
                    $failedRecs = $harvest->failedHarvests->sortBy(['updated_at', 'desc']);
                    $lastFailed = $failedRecs->first();
                    if (!is_null($harvest->rawfile) && ($lastFailed->error_id == 3030 || $lastFailed->error_id == 9030)) {
                        $deleteFile = true;
                    }

                    // Cleanup failed records
                    foreach ($failedRecs as $idx => $failed) {
                        if ($idx==0 && in_array($failed->error_id, [3030,9030])) {
                            continue;
                        }
                        $failed->delete();
                    }
                }
                if ( !($harvest->attempts > 0) ) $harvest->attempts = 1;

            // Clear all saved JSON files for Fail and Stopped status
            } else {
                $deleteFile = true;
            }
        ...
        foreach ($file_glob as $jsonFile) {
            ...
            if (!$deleteFile) {
                ...
                // If success with existing failed() records, check the last (most recent) one for 3030 & 9030
                } else if ($harvest->status == 'Success') {
                    $lastFailed = $harvest->failedHarvests->sortBy(['updated_at', 'desc'])->first();
                    if ($lastFailed->error_id == 3030 || $lastFailed->error_id == 9030) $deleteFile = true;
