 ...
 */
class HarvestBatch extends Command
{
    /**
     * The name and signature for the HarvestBatch processing console command.
     * @var string
     */
    protected $signature = 'ccplus:harvestbatch {consortium : The Consortium ID or key-string}
     ...
     * ----------------------------
     *   ccplus:harvestbatch is intended to be as an artisan command for bulk-loading report data without
    ...
    public function handle()
    {
       ...
       // Get all active records from the consortium providers table
        if ($prov_id == 0) {
            $connections = Connection::with('globalProv','reports')->where('is_active',true)->get();
        } else {
            $connections = Connection::with('globalProv','reports')->where('is_active',true)->where('global_id',$prov_id)->get();
        }

       // Get credentials for all the global_ids based on the
        $global_ids = $connections->pluck('global_id')->toArray();
        $credentials = Credential::with('institution', 'provider')->where('status', 'Enabled')
       ...
       // Loop through all credentials for this provider
        foreach ($credentials as $credential) {
           // Skip this credential if we're just processing a single inst and the IDs don't match
            if ( (!$credential->institution->is_active) || ($inst_id!=0 && $credential->inst_id!=$inst_id)) {
                continue;
            }

           // Create a new processor object
            $C5processor = new Counter5Processor($credential->prov_id, $credential->inst_id, $begin, $end, $replace);

           // Get (conso) connections for the global provider
            $conns = $connections->where('global_id',$credential->prov_id);
            $conso_connection = $conns->where('inst_id',1)->first();
            $conso_reports = ($conso_connection) ? $conso_connection->reports->pluck('id')->toArray() : [];

           // Loop through matching (conso) providers
            $this->line("Processing: " . $credential->provider->name);
            foreach ($conns as $cnx) {
               // If running as "Auto" and today is not the day to run, skip silently to next provider
                if ($this->option('auto') && $cnx->GlobalProv->day_of_month != date('j')) {
                    continue;
                }

               // if the provider is inst-assigned, skip it on mismatch to credential inst_id
                if ($cnx->inst_id>1 && $cnx->inst_id != $credential->inst_id) {
                    continue;
                }

                // De-dupe provider reports against $conso_reports and skip this provider if no unique reports
                $prov_report_ids = $cnx->reports->pluck('id')->toArray();
                ...
                $registry = $cnx->globalProv->default_registry();
                ...
                foreach ($reports as $report) {
                    ...
                    if ($cnx->inst_id>1 && in_array($report->id,$conso_reports)) {
                        continue;
                    }
                   ...
                   // Create new HarvestLog record; if one already exists, use it instead
                    try {
                        $harvest = HarvestLog::create(['status' => 'Active', 'credentials_id' => $credential->id,
                        ...
                        if ($errorCode == '1062') {
                            ...
                            $this->line('Harvest ' . '(ID:' . $harvest->id . ') already defined for credential: ' .
                                        $credential->id . ', ' . $report->name . ':' . $yearmon . ').');
                   ...
                   // Use hardcoded sleep and retry limits
                   ...
                   // Process the report and save in the database
                    if ($valid_report) {
                        ...
                        if ($_status = 'Success') {
                            ...
                            // Keep track last successful for this credential
                            if ($yearmon != $credential->last_harvest) {
                                $credential->last_harvest = $yearmon;
                                $credential->update();
                            }
